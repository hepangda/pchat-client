...
UIResetpwd::UIResetpwd(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::UIResetpwd)
{
    ui->setupUi(this);
...
UIResetpwd::~UIResetpwd()
{
    delete ui;
}


string UIResetpwd::JsonlizeUI()
{
    string ret;
    Json::Value root;
...
void UIResetpwd::on_txtChkpasswd_editingFinished()
{
...
void UIResetpwd::on_btnRegister_clicked()
{
    int ret;
    if ((ret = checkUI()) != 0) {
        QMessageBox::warning(NULL, "重置密码失败", "输入的信息不合法:\n*" + failedString[ret - 1],
    ...
    if (rev["res"].asInt() == 0) {
        QMessageBox::information(NULL, "PChat", "重置密码成功！\n您可以使用新的密码登录！",
                                 QMessageBox::Ok, QMessageBox::Ok);
        close();
    } else {
        QMessageBox::critical(NULL, "PChat", "重置密码失败！\n请检查密码找回信息是否正确。",
                              QMessageBox::Ok, QMessageBox::Ok);
    }
}

void UIResetpwd::on_btnCancel_clicked()
{
    close();
}
